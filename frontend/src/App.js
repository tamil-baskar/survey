...
function getBestMatch(description, keywords) {
  ...
  // First, try to find exact matches for multi-word keywords
  for (const keyword of keywords) {
    if (keyword.includes(' ') && desc.includes(keyword)) {
      return keyword;
    }
  }
  ...
  // If no exact matches, fall back to the original similarity check
  const similarity = (s1, s2) => {
    ...
    if (commonTypos[shorter]) {
      for (const typo of commonTypos[shorter]) {
        if (desc.includes(typo)) return 0.9;
      }
    }
...
function generateQuestions(description) {
  ...
  const templates = [
    ...
    {
      keywords: ['customer satisfaction', 'client happiness', 'service feedback', 'satisfaction survey'],
      questions: [
